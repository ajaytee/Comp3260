...
public class AddRoundKey {
	...
	private static byte[][] generateSubkeys(byte[] key) {
		...
		while (i < 176) { // need 176 bytes of key
		    ...
		    for (int j = 0; j < 4; j++) {
		        temp[j] = subkeys[round][(j + i - 4) - (round * 16)]; // tempBytes = last 4 bytes of previous subkey
		        // for Round 0: (i = 16)
		    ...
		    for (int j = 0; j < 4; j++) {
		        ...
		        int tempInt = (int) temp[j];
		            
		        // change subkey value from byte to int
		        int subkeyInt = (int) subkeys[round - 2][i - ((round - 1) * 16)];
