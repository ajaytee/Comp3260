...
public class ShiftRows {
	...
	// Accepts String as input, converts to Matrix and shifts rows, returns String as output
	// Call initialise with String first, then call shift("encryption") or shift("decryption") to execute the shift
	String inputString = "";
	...
	public String[][] convertToMatrix(String binaryString) {
		...
		for (int i=0; i<16; i++) {
			hexStrings[i] = Integer.toHexString(Integer.parseInt(byteStrings[i], 2));
		}
	...
	public String shift(String option) {
		...
		if (option.equals("encyption")) {
			// shift left
			for (int i=1; i<4; i++) { 		// initialise i at 1 as Row 0 remains unchanged
				for (int j=0; j<4; j++) {	// i = rows, j = columns
					...
					int newPos = j - i;
					if (newPos < 0) {
						newPos = newPos + 4; // if new column = -1, it actually means 3 as its a circular shift
					}
