...
public class Encrypter {
    ...
    public byte[][][] encryptByteArray(byte[] inputArray, byte[] key,
        ...
        output[0] = Helper.copyByteMatrix(roundKeyAdder.addRoundKey(input, 0));
        String formattedOutput = Helper.matrixToHexString(output[0]);
        for (int round = 1; round < 11; round++) {
            byte[][] tmp = new byte[input.length][input[0].length];
            tmp = Helper.copyByteMatrix(output[round - 1]);
            formattedOutput = Helper.matrixToHexString(tmp);
            if (!type.isSkipSubBytes()) {
                tmp = byteSubstituter.substituteBytes(tmp, false);
            }
            formattedOutput = Helper.matrixToHexString(tmp);
            ...
            if (!type.isSkipAddRoundKey()) {
                tmp = Helper.copyByteMatrix(roundKeyAdder.addRoundKey(tmp,
                        round));
            }
            formattedOutput = Helper.matrixToHexString(tmp);
    ...
    private int[][] avalanche(String plainText, String[] encryptedText,
        ...
        for (int i = 0; i < plainText.length(); i++) {
            ...
            for (int round = 0; round < 11; round++) {
                 ...
                 * count of changes to plainTextChanges and keyChanges
                 */
